...
import frc.robot.commands.changeFixedPositionCommand;
...
public class RobotContainer {
    ...
    Limelight limelight_right = new Limelight("limelight-left");
    ...
    public RobotContainer() {
        ...
        autoChooser.addRoutine("LowBallMiddleAuto", this::LowBallMiddleAuto);
...
//Didnt finish 
    ...
    private void configureBindings() {
        ...
        Player1.a().whileTrue(new ExtensionCommand(extension, Constants.Positions.HighBallExtension));
        ...
        //high ball
        BlueTop.onTrue(new ParallelCommandGroup(
                new PivotCommand(pivot, Constants.Positions.HighBallPivot),
                new ExtensionCommand(extension, Constants.Positions.HighBallExtension),
        ...
        //Player1.povDown().onTrue(new changeFixedPositionCommand("elevator", -.05));
        //Player1.povUp().onTrue(new changeFixedPositionCommand("elevator", .05));
        Player1.povDown().whileTrue(drivetrain.applyRequest(() -> driveRobotCentric
            .withVelocityX(-.2)
            .withVelocityY(0)
            .withRotationalRate(0)
        ));
    ...
    private AutoRoutine LowBallMiddleAuto() {
        try {
            if (DriverStation.getAlliance().get() == DriverStation.Alliance.Blue)
                autoFactory = drivetrain.createAutoFactoryBlue();
            else    
        ...
        AutoRoutine routine = autoFactory.newRoutine("LowBallMiddleAuto");
        AutoTrajectory traj = routine.trajectory("LowBallMiddle1");
        traj.done().onTrue(new SequentialCommandGroup(
            new PivotCommand(pivot, Constants.Positions.LowBallPivot),
            new WristCommand(Wrist, Constants.Positions.LowBallWrist)
        ));
        routine.active().onTrue(Commands.sequence(
